...
function GraphCanvasInner() {
  ...
  useEffect(() => {
    if (
      thompsonState.isTemplateCorrect === true &&
      prevTemplateCorrect.current !== true
    ) {
  ...
  const thompsonDisplayNFA = useMemo(() => {
    ...
    if (isTemplateCorrect === true)
      return steps[currentStepIndex]?.nfaAfter ?? null;
    return currentStepIndex > 0
      ? (steps[currentStepIndex - 1]?.nfaAfter ?? null)
      : null;
  ...
  const thompsonHighlight = useMemo(() => {
    ...
    return {
      ...
      newTransitionIds: step.newTransitionIds,
    };
  }, [isThompson, thompsonState]);
  ...
  const currentPath =
    ...
    stateEliminationState.currentPathIndex <
      stateEliminationState.currentPathUpdates.length
  ...
  const highlight = useMemo(
    ...
    [
      isConverting,
      stateEliminationState.stateToRemove,
      currentPath,
      stateEliminationState.highlightedR,
    ],
  );

  const { nodes: layoutedNodes, edges } = useGraphLayout(graphData, {
    highlight,
    thompsonHighlight,
  });
  ...
  const [edgeBends, setEdgeBends] = useState<
    Record<string, { x: number; y: number }>
  >({});
