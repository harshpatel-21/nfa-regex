...
describe("GraphCanvas branch coverage scenarios", () => {
  ...
  it("triggers fitView when template correctness transitions to true", () => {
    ...
    const state = {
      ...
      thompsonState: {
        ...
        steps: [
          {
            nfaAfter: { states: [], transitions: [], alphabet: [] },
            newStateIds: [],
            newTransitionIds: [],
          },
        ],
  ...
  it("computes thompsonHighlight with newStateIds when template is correct in stepping phase", () => {
    ...
    expect(
      (options as { thompsonHighlight: unknown }).thompsonHighlight,
    ).toMatchObject({
      newStateIds: ["q0"],
      newTransitionIds: ["t0"],
    });
  ...
  it("preserves displayed positions for remaining nodes when a state is eliminated", () => {
    ...
    expect(capturedNodes?.find((n) => n.id === "A")?.position).toEqual({
      x: 10,
      y: 20,
    });
    expect(capturedNodes?.find((n) => n.id === "B")?.position).toEqual({
      x: 50,
      y: 60,
    });
  });
  ...
  it("uses dagre position for a node that is new to the display during state elimination", () => {
    ...
    expect(capturedNodes?.find((n) => n.id === "D")?.position).toEqual({
      x: 77,
      y: 88,
    });
  ...
  it("preserves existing node positions when a Thompson step adds new nodes", () => {
    ...
    expect(capturedNodes?.find((n) => n.id === "C")?.position).toEqual({
      x: 33,
      y: 44,
    });
  });
