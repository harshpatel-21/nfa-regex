...
vi.mock('../conversion/StateSelector', () => ({
  StateSelector: () => <div data-testid="state-selector-mock">state selector</div>,
}))

vi.mock('../conversion/PathUpdateForm', () => ({
  PathUpdateForm: () => <div data-testid="path-update-form-mock">path form</div>,
}))
...
describe('StateEliminationPanel branch rendering', () => {
  ...
  it('renders selecting-state controls and revert button when elimination exists in history', () => {
    ...
    expect(screen.getByRole('button', { name: /Revert Last Elimination/i })).toBeInTheDocument()
  })
  ...
  it('renders step-complete unreachable-state message when there are zero path updates', () => {
    ...
    expect(screen.getByRole('button', { name: /Complete Elimination/i })).toBeInTheDocument()
  })
})