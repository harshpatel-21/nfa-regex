...
describe("FormulaDisplay — info tooltips", () => {
  ...
  it("hides the R₁ description when its info button is clicked a second time", async () => {
    ...
    await userEvent.click(r1InfoBtn!);
    expect(
      screen.queryByText(/Transition from q0 to q1/i),
  ...
  it("shows the R₄ description when its info button is clicked", async () => {
    ...
    expect(
      screen.getByText(/Existing direct transition from q0 to q2/i),
    ).toBeInTheDocument();
  });
...
describe("FormulaDisplay — hover handlers", () => {
  it("fires onMouseEnter and onMouseLeave on an R-value box without throwing", () => {
    const { container } = render(<FormulaDisplay {...defaultProps} />, {
      wrapper: Wrapper,
    });
