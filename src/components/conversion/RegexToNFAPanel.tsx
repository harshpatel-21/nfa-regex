...
function RegexHighlight({ regex, start, end }: { regex: string; start: number; end: number }) {
  ...
  return (
    <span className="font-mono text-sm tracking-wide">
      ...
      <span className="rounded bg-amber-200 px-0.5 font-semibold text-amber-900">{match}</span>
...
const TEMPLATE_LABELS: Record<ThompsonTemplate, { label: string; description: string }> = {
  symbol: { label: 'Base — symbol', description: 'A single character (e.g. a, b, 0)' },
  epsilon: { label: 'Base — ε', description: 'The empty string ε' },
  union: { label: 'Union  (r₁ + r₂)', description: 'Choice between two sub-expressions' },
  concat: { label: 'Concatenation  (r₁r₂)', description: 'One sub-expression followed by another' },
  star: { label: 'Kleene Star  (r*)', description: 'Zero or more repetitions' },
}

const ALL_TEMPLATES: ThompsonTemplate[] = ['symbol', 'epsilon', 'union', 'concat', 'star']
      ...
      <tbody>
        {rows.map((row, i) => (
          <tr key={i} className="border-t border-gray-100">
            <td className="py-0.5 pr-3 text-amber-700 font-semibold">{row.sourceLabel}</td>
            <td className="py-0.5 pr-3 text-blue-700">{row.symbol}</td>
            <td className="py-0.5 text-amber-700 font-semibold">{row.targetLabel}</td>
          </tr>
        ))}
      </tbody>
    </table>
    ...
    return (
      <div className="flex flex-col gap-4 p-4">
        ...
        <div className="flex flex-col gap-2">
          <label className="text-xs font-medium text-gray-600">Regular expression</label>
          <input
            type="text"
            value={localRegex}
          ...
          <p className="text-xs text-gray-400">
            ...
            <span className="font-mono">*</span> (Kleene star), implicit concatenation,{' '}
            <span className="font-mono">( )</span> grouping, <span className="font-mono">ε</span>
          </p>
        </div>
    ...
    return (
      <div className="flex flex-col gap-4 p-4">
        ...
        <div className="rounded border border-green-200 bg-green-50 p-3 text-center">
          <p className="text-xs font-medium text-green-700 mb-1">Construction complete!</p>
          <p className="text-sm font-mono text-green-900 font-semibold">{regex}</p>
        </div>
        ...
        <div className="rounded border border-gray-200 bg-gray-50 p-3 text-xs text-gray-600">
          ...
          <p>
            States: {finalStep?.nfaAfter.states.length ?? 0} &nbsp;·&nbsp; Transitions:{' '}
            ...
            {ALL_TEMPLATES.map((t) => {
              ...
              return (
                ...
                >
                  ...
                  <span>
                    <span className="font-medium">{TEMPLATE_LABELS[t].label}</span>
          ...
          {currentStep.expectedTransitions.length > 0 && (
            <div className="rounded border border-amber-100 bg-amber-50 px-3 py-2">
              <p className="text-xs font-medium text-amber-800 mb-2">
                New transitions added{currentStep.newStateIds.length > 0 ? ` (states ${currentStep.newStateIds
                  .map((id) => currentStep.nfaAfter.states.find((s) => s.id === id)?.label ?? id)
