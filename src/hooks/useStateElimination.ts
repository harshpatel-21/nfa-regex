...
export function useStateElimination() {
  ...
  const selectStateToRemove = useCallback(
    (stateId: StateId) => {
      ...
      const pathUpdates = computePathUpdates(stateEliminationState.gtg, stateId)
      stateEliminationDispatch({ type: 'SET_PATH_UPDATES', payload: pathUpdates })
    },
  ...
  const completeElimination = useCallback(() => {
    if (!stateEliminationState.gtg || !stateEliminationState.stateToRemove) return
