import type { NFA, State, Transition, ThompsonStep, ThompsonTemplate, ThompsonTransitionRow } from './types'
...
type UnionNode = { type: 'union'; left: ASTNode; right: ASTNode; start: number; end: number }
type ConcatNode = { type: 'concat'; left: ASTNode; right: ASTNode; start: number; end: number }
...
function parseTerm(p: ParserState): ASTNode {
  ...
  while (
    ...
    node = { type: 'concat', left: node, right, start: node.start, end: right.end }
  }
  return node
}
...
function buildSteps(
  ...
  switch (node.type) {
    ...
    case 'epsilon': {
      ...
      const t: Transition = { id: newTransId(), source: sId, target: fId, symbol: sym }
      ...
      const template: ThompsonTemplate = node.type === 'symbol' ? 'symbol' : 'epsilon'
      ...
      return { fragment: { startId: sId, finalId: fId }, steps: [step], nfa: rawNFA }
    }
    ...
    case 'union': {
      ...
      const t1: Transition = { id: newTransId(), source: qsId, target: left.fragment.startId, symbol: EPSILON }
      const t2: Transition = { id: newTransId(), source: qsId, target: right.fragment.startId, symbol: EPSILON }
      const t3: Transition = { id: newTransId(), source: left.fragment.finalId, target: qfId, symbol: EPSILON }
      const t4: Transition = { id: newTransId(), source: right.fragment.finalId, target: qfId, symbol: EPSILON }
      ...
      const lsLabel = left.nfa.states.find((s) => s.id === left.fragment.startId)?.label ?? '?'
      const lfLabel = left.nfa.states.find((s) => s.id === left.fragment.finalId)?.label ?? '?'
      const rsLabel = right.nfa.states.find((s) => s.id === right.fragment.startId)?.label ?? '?'
      const rfLabel = right.nfa.states.find((s) => s.id === right.fragment.finalId)?.label ?? '?'

      const subExpr = input.slice(node.start, node.end + 1)
      const explanation =
        `Union template for '${subExpr}'. Add new start ${qsLabel} with ε-transitions to both sub-NFA starts (${lsLabel}, ${rsLabel}), and new final ${qfLabel} receiving ε-transitions from both sub-NFA finals (${lfLabel}, ${rfLabel}).`

      const expectedTransitions: ThompsonTransitionRow[] = [
        { id: t1.id, sourceLabel: qsLabel, symbol: EPSILON, targetLabel: lsLabel },
        { id: t2.id, sourceLabel: qsLabel, symbol: EPSILON, targetLabel: rsLabel },
        { id: t3.id, sourceLabel: lfLabel, symbol: EPSILON, targetLabel: qfLabel },
        { id: t4.id, sourceLabel: rfLabel, symbol: EPSILON, targetLabel: qfLabel },
      ]
    ...
    case 'concat': {
      ...
      const nfaAfter = markFragment(rawNFA, left.fragment.startId, right.fragment.finalId)
      ...
      const subExpr = input.slice(node.start, node.end + 1)
      const explanation =
        `Concatenation template for '${subExpr}'. Connect the final state of the left sub-NFA (${lfLabel}) to the start state of the right sub-NFA (${rsLabel}) via an ε-transition.`
      ...
      const step: ThompsonStep = {
        ...
        explanation,
        expectedTransitions: [
          { id: t.id, sourceLabel: lfLabel, symbol: EPSILON, targetLabel: rsLabel },
        ],
      }

      return {
        fragment: { startId: left.fragment.startId, finalId: right.fragment.finalId },
        steps: [...left.steps, ...right.steps, step],
        nfa: rawNFA,
      }
    }
    ...
    case 'star': {
      ...
      const t1: Transition = { id: newTransId(), source: qsId, target: child.fragment.startId, symbol: EPSILON }
      const t2: Transition = { id: newTransId(), source: child.fragment.finalId, target: qsId, symbol: EPSILON }
      ...
      const csLabel = child.nfa.states.find((s) => s.id === child.fragment.startId)?.label ?? '?'
      const cfLabel = child.nfa.states.find((s) => s.id === child.fragment.finalId)?.label ?? '?'

      const subExpr = input.slice(node.start, node.end + 1)
      const explanation =
        `Kleene Star template for '${subExpr}'. Add new state ${qsLabel} (both start and final, allowing zero repetitions). ε-transition forward (${qsLabel} → ${csLabel}) enters the sub-NFA; ε-transition back (${cfLabel} → ${qsLabel}) allows repetition.`
      ...
      const step: ThompsonStep = {
        ...
        explanation,
        expectedTransitions: [
          { id: t1.id, sourceLabel: qsLabel, symbol: EPSILON, targetLabel: csLabel },
          { id: t2.id, sourceLabel: cfLabel, symbol: EPSILON, targetLabel: qsLabel },
        ],
      }
...
export function buildThompsonSteps(regex: string): { steps: ThompsonStep[]; error?: string } {
  stateCounter = 0
  transitionCounter = 0
  ...
  try {
    ...
    if (p.pos < p.input.length) {
      throw new Error(`Unexpected character '${p.input[p.pos]}' at position ${p.pos}`)
    }
  } catch (e) {
...
function collectAlphabet(node: ASTNode): string[] {
  switch (node.type) {
    ...
    case 'union':
    case 'concat':
      return [...new Set([...collectAlphabet(node.left), ...collectAlphabet(node.right)])]
    case 'star':
